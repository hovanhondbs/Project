  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log('Recent sets raw data:', recentRes.data);

        const processedSets = recentRes.data
          .filter(item => item.setId && typeof item.setId === 'object')
          .map(item => ({
            ...item.setId,
            userId: item.setId.userId || { username: 'Unknown' },
            lastViewed: item.lastViewed, // Giữ lại thông tin ngày xem
            // Thêm trường createdAt nếu chưa có
            createdAt: item.setId.createdAt || new Date()
          }));

        console.log('Processed recent sets:', processedSets);
