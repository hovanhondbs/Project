...
function UserLibrary() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE}/api/user/${storedUserId}`)
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE}/api/flashcards/user/${storedUserId}`)
  ...
  useEffect(() => {
    if (userData?.role === 'Teacher') {
      axios
        .get(`${API_BASE}/api/classrooms/by-user/${storedUserId}`)
  ...
  useEffect(() => {
    if (userData?.role === 'User') {
      axios
        .get(`${API_BASE}/api/classrooms/joined/${storedUserId}`)
        .then((res) => setClasses(res.data))
                    ...
                    >
                      <Link to={`/classes/${cls._id}`} className="block">
                        <div className="flex justify-between items-center text-sm text-gray-600 mb-2">
                          ...
                          <div className="flex items-center">
                            {(() => {
                              const name = cls.createdBy?.username || userData?.username || 'You';
                              const url = abs(cls.createdBy?.avatar || userData?.avatar || '');
                              const initials = getAvatarInitials(name);
                              return url ? (
                                <img
                                  src={url}
                            ...
                            <span>{cls.createdBy?.username || userData?.username}</span>
                          </div>
                        </div>
